...
public class StringUtils extends org.apache.commons.lang.StringUtils{
	...
	public static String substringByByte(String str, int begin, int end) {
		...
		if (isEmpty(str) || begin < 0 || str.getBytes(charset).length < end) {
			return str;
		}
		...
		int len = 0;
		for (int i = begin; len < maxLen; i++) {
			char c = str.charAt(i);
			len += String.valueOf(c).getBytes(charset).length;
			// len == maxLen时仍append,然后循环结束
			if (len > maxLen) {
				break;
			}
			buf.append(c);
